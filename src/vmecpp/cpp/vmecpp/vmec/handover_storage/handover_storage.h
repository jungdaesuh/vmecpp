...
class HandoverStorage {
 public:
  ...
  // Access: rmncc_i(thread, mn) for element access
  //
  ...
  // =========================================================================
  // Tri-diagonal matrix arrays: RowMatrixXd [mn, j]
  // Radial dimension (j) is contiguous for optimal solver access because
  // RowMatrixXd is row-major.
  //
  // RHS arrays: vector of RowMatrixXd [mn][basis, j]
  // For each mode mn, we have a matrix where each row corresponds to a basis
  // function and columns are radial points.
